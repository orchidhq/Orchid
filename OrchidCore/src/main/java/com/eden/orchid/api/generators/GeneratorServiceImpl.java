...
public final class GeneratorServiceImpl implements GeneratorService {
    ...
    @Description("By default Generators run serially in priority order, but this flag will allow all generators to beindexed in parallel for improved performance. Not suitable for builds where later generators depend on the pages indexed by higher-priority generators.")
    ...
    public String[] getGeneratorKeys(String[] include, String[] exclude) {
        return getFilteredGenerators(getFilteredGenerators(false), include, exclude).map(OrchidGenerator::getKey).toArray(String[]::new);
    }
