...
public final class PebbleWrapperTemplateTag implements TokenParser {
    ...
    public RenderableNode parse(Token token, Parser parser) throws ParserException {
        switch (type) {
            case Simple:  return new SimpleTagParser( contextProvider, name, tagParameters, tagClass).parse(token, parser);
            case Content: return new ContentTagParser(contextProvider, name, tagParameters, tagClass).parse(token, parser);
            case Tabbed:  return new TabbedTagParser( contextProvider, name, tagParameters, tagClass, tabParameters, tabClass).parse(token, parser);
            default:      throw  new IllegalArgumentException("Tag type must be a valid type.");
        }
    }
